#!/usr/bin/env python
# coding=utf-8
from taurusxradius.txradius.radius.packet import tools
from taurusxradius.txradius.radius.packet import AuthPacket
from taurusxradius.txradius.radius.packet import AcctPacket
from taurusxradius.txradius.radius.packet import CoAPacket
from taurusxradius.txradius.radius.packet import AccessRequest
from taurusxradius.txradius.radius.packet import AccessAccept
from taurusxradius.txradius.radius.packet import AccountingRequest
from taurusxradius.txradius.radius.packet import AccountingResponse
from taurusxradius.txradius.radius.packet import CoARequest
from taurusxradius.txradius.mschap import mschap, mppe
import time
import binascii
import datetime
import hashlib
...
PacketStatusTypeMap = {1: 'AccessRequest',
 2: 'AccessAccept',
 3: 'AccessReject',
 4: 'AccountingRequest',
...
class CoAMessage(CoAPacket, ExtAttrMixin):
    ...
    def __init__(self, code = CoARequest, id = None, secret = six.b(''), authenticator = None, **attributes):
...
class AuthMessage(AuthPacket, ExtAttrMixin):
    ...
    def CreateReply(self, **attributes):
        return AuthMessage(AccessAccept, self.id, self.secret, self.authenticator, dict=self.dict, **attributes)
    ...
    def verifyMsChapV2(self, userpwd):
        ...
        nt_resp = mschap.generate_nt_response_mschap2(authenticator_challenge, peer_challenge, _user_name, userpwd)
        if nt_resp == nt_response:
            auth_resp = mschap.generate_authenticator_response(userpwd, nt_response, peer_challenge, authenticator_challenge, _user_name)
            ...
            self.resp_attrs['Reply-Message'] = 'E=691 R=1 C=%s V=3 M=<password error>' % ('\x00' * 32)
            return False
    ...
    def get_pwd_type(self):
        if 'MS-CHAP-Challenge' in self:
            if 'MS-CHAP-Response' in self:
                return 'mschapv1'
            if 'MS-CHAP2-Response' in self:
                return 'mschapv2'
        else:
            if 'CHAP-Password' in self:
                return 'chap'
            return 'pap'
    ...
    def is_valid_pwd(self, userpwd):
        ...
        try:
            ...
            if pwd_type == 'mschapv1':
                return False
            if pwd_type == 'mschapv2':
...
class AcctMessage(AcctPacket, ExtAttrMixin):
    ...
    def __init__(self, code = AccountingRequest, id = None, secret = six.b(''), authenticator = None, **attributes):
    ...
    def CreateReply(self, **attributes):
        return AcctMessage(AccountingResponse, self.id, self.secret, self.authenticator, dict=self.dict, **attributes)
    ...
    def get_nas_port(self):
        try:
            return tools.DecodeInteger(self.get(5)[0]) or 0
        except:
            return 0
    ...
    def get_acct_output_octets(self):
        try:
            return tools.DecodeInteger(self.get(43)[0]) or 0
        except:
            return 0
    ...
    def get_acct_input_packets(self):
        try:
            return tools.DecodeInteger(self.get(47)[0]) or 0
        except:
            return 0
    ...
    def get_acct_input_gigawords(self):
        try:
            return tools.DecodeInteger(self.get(52)[0]) or 0
        except:
            return 0
    ...
    def get_input_total(self):
        ...
        gl = decimal.Decimal(self.get_acct_input_gigawords()) * decimal.Decimal(4194304)
        tl = bl + gl
    ...
    def get_output_total(self):
        ...
        gl = decimal.Decimal(self.get_acct_output_gigawords()) * decimal.Decimal(4194304)
        tl = bl + gl
        return int(tl.to_integral_value())
    ...
    def get_event_timestamp_str(self, timetype = 0):
        try:
            ...
            return datetime.datetime.fromtimestamp(_time - 28800).strftime('%Y-%m-%d %H:%M:%S')
        except:
            return ''
    ...
    def get_ticket(self):
        return dict(account_number=self.get_user_name(), mac_addr=self.get_mac_addr(), nas_addr=self.get_nas_addr(), nas_port=self.get_nas_port(), service_type=self.get_service_type(), framed_ipaddr=self.get_framed_ipaddr(), framed_netmask=self.get_framed_netmask(), nas_class=self.get_nas_class(), session_timeout=self.get_session_timeout(), calling_station_id=self.get_calling_stationid(), acct_status_type=self.get_acct_status_type(), acct_input_octets=self.get_acct_input_octets(), acct_output_octets=self.get_acct_output_octets(), acct_session_id=self.get_acct_sessionid(), acct_session_time=self.get_acct_sessiontime(), acct_input_packets=self.get_acct_input_packets(), acct_output_packets=self.get_acct_output_packets(), acct_terminate_cause=self.get_acct_terminate_cause(), acct_input_gigawords=self.get_acct_input_gigawords(), acct_output_gigawords=self.get_acct_output_gigawords(), event_timestamp=self.get_event_timestamp_str(), nas_port_type=self.get_nas_port_type(), nas_port_id=self.get_nas_portid())

    def get_billing(self):
        return dict(nas_id=self.get_nas_id(), acct_session_id=self.get_acct_sessionid(), username=self.get_user_name(), mac_addr=self.get_mac_addr(), nas_addr=self.get_nas_addr(), nas_port=self.get_nas_port(), nas_port_id=self.get_nas_portid(), nas_port_type=self.get_nas_port_type(), nas_class=self.get_nas_class(), framed_ipaddr=self.get_framed_ipaddr(), framed_netmask=self.get_framed_netmask(), session_timeout=self.get_session_timeout(), acct_input_total=self.get_input_total(), acct_output_total=self.get_output_total(), acct_start_time=self.get_acct_start_time(), acct_session_time=self.get_acct_sessiontime(), acct_input_packets=self.get_acct_input_packets(), acct_output_packets=self.get_acct_output_packets(), acct_terminate_cause=self.get_acct_terminate_cause())