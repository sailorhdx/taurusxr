...
class HttpServer(cyclone.web.Application):
    ...
    def __init__(self, config = None, dbengine = None, **kwargs):
        ...
        settings = dict(cookie_secret='12oETzKXQAGaYdkL5gEmGeJJFuYh7EQnp2XdTP1o/Vo=', login_url='/ssportal/login', template_path=os.path.join(os.path.dirname(taurusxradius.__file__), 'views'), static_path=os.path.join(os.path.dirname(taurusxradius.__file__), 'static'), xsrf_cookies=True, config=self.config, debug=self.config.system.debug, xheaders=True)
        ...
        load_handlers(handler_path=os.path.join(os.path.abspath(os.path.dirname(__file__)), 'ssportal'), pkg_prefix='taurusxradius.modules.ssportal', excludes=['webserver', 'radius'])
