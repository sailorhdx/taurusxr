...
from taurusxradius.taurusxlib import utils, dispatch, redis_cache, logger
from taurusxradius.modules.events.settings import ACCOUNT_NEXT_EVENT
from taurusxradius.modules.events.settings import UNLOCK_ONLINE_EVENT
from taurusxradius.modules.dbservice import BaseService
from taurusxradius.modules.dbservice import logparams
...
class AccountRenew(BaseService):
    ...
    def check_vcard(self, vcard, vcard_pwd, product):
        ...
        if product.product_policy not in (BOMonth, BOFlows, BOTimes):
            self.last_error = u'当前资费不支持此充值卡(仅支持买断包月,买断流量,买断时长类型资费)'
            return False
