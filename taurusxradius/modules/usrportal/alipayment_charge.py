...
from taurusxradius.modules.usrportal import alipayment_new
...
@permit.route('/usrportal/product/charge')

class UsrPortalChargeOrderHandler(alipayment_new.BasicOrderHandler):
    """ 发起续费支付
    """
    ...
    def get(self):
        ...
        self.render('profile_charge_modal_form.html', form=form)
...
@permit.route('/usrportal/product/charge/alipay')

class UsrPortalProductRenewHandler(BaseHandler):
    """ 支付信息确认，跳转支付宝
    """
    ...
    def get(self):
        ...
        try:
            formdata.order_id = order_id
            self.render('profile_charge_alipay.html', formdata=formdata)
        except Exception as err:
            logger.exception(err)
