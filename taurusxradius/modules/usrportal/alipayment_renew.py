...
class UsrPortalRenewOrderHandler(alipayment_new.BasicOrderHandler):
    """ 发起续费支付
    """
    ...
    def get(self):
        ...
        self.render('profile_renew_modal_form.html', form=form)
...
@permit.route('/usrportal/product/renew/alipay')
...
class UsrPortalProductRenewHandler(BaseHandler):
    ...
    def get(self):
        ...
        try:
            formdata.order_id = order_id
            self.render('profile_renew_alipay.html', formdata=formdata)
        except Exception as err:
            logger.exception(err)
