...
class CacheManager(object):
    ...
    def print_hit_stat(self, first_delay = 0):
        ...
        logstr = '\n\n----------------------- cache stat ----------------------\n#  cache name              : {0}\n#  visit cache total       : {1}\n#  add cache total         : {2}\n#  hit cache total         : {3}\n#  update cache total      : {4}\n#  delete cache total      : {5}\n#  current db cache total  : {6}\n---------------------------------------------------------\n\n'.format(self.cache_name, self.get_total, self.set_total, self.hit_total, self.update_total, self.delete_total, self.count())
    ...
    def update(self, key, value, expire = 0):
        ...
        with self.dbengine.begin() as conn:
            _time = expire > 0 and int(time.time()) + int(expire) or 0
            try:
                conn.execute(_sql('update %s \n                                    set _value=:value, _time=:time\n                                    where _key=:key' % self.cache_table), key=key, value=raw_data, time=_time)
            except:
                import traceback
                traceback.print_exc()