...
class SimpleLogger:
    ...
    def __init__(self, config, name = 'taurusxr'):
        self.name = name
        self.setup(config)
...
class Logger:
    ...
    def setup(self, config):
        ...
        self.formatter = logging.Formatter(u'%(asctime)s {0} %(name)s %(levelname)-8s %(message)s'.format(self.syslog_shost), '%b %d %H:%M:%S')
