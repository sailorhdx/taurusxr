...
@permit.route('/admin/customer/billing', u'用户计费日志', MenuUser, order=6.0, is_menu=True)
...
class CustomerBillingHandler(BaseHandler):
    ...
    def post(self):
        ...
        _query = self.db.query(models.TrBilling, models.TrCustomer.node_id, models.TrNode.node_name).filter(models.TrBilling.account_number == models.TrAccount.account_number, models.TrCustomer.customer_id == models.TrAccount.customer_id, models.TrNode.id == models.TrCustomer.node_id)
        if node_id:
        ...
        if self.request.path == '/admin/customer/billing':
            return self.render('billing_list.html', node_list=opr_nodes, page_data=self.get_page_data(_query), **self.get_params())
        elif self.request.path == '/admin/customer/billing/export':
            data = Dataset()
            data.append((u'区域', u'上网账号', u'BAS地址', u'会话编号', u'记账开始时间', u'会话时长', u'已扣时长', u'已扣流量', u'应扣费用', u'实扣费用', u'剩余余额', u'剩余时长', u'剩余流量', u'是否扣费', u'扣费时间'))
            _f2y = utils.fen2yuan
...
@permit.route('/admin/customer/billing/export', u'用户计费日志导出', MenuUser, order=3.0001)
