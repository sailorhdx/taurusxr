...
@permit.suproute('/admin/customer/acceptlog', u'用户受理日志', MenuUser, order=3.0, is_menu=True)
...
class CustomerAcceptLoggerHandler(BaseHandler):
    ...
    def post(self):
        ...
        _query = self.db.query(models.TrAcceptLog.id, models.TrAcceptLog.accept_type, models.TrAcceptLog.accept_time, models.TrAcceptLog.accept_desc, models.TrAcceptLog.operator_name, models.TrAcceptLog.accept_source, models.TrAcceptLog.account_number, models.TrCustomer.node_id, models.TrNode.node_name).filter(models.TrAcceptLog.account_number == models.TrAccount.account_number, models.TrCustomer.customer_id == models.TrAccount.customer_id, models.TrNode.id == models.TrCustomer.node_id)
        if operator_name:
        ...
        if self.request.path == '/admin/customer/acceptlog':
            return self.render('acceptlog_list.html', page_data=self.get_page_data(_query), node_list=opr_nodes, type_map=type_map, get_orderid=(lambda aid: self.db.query(models.TrCustomerOrder.order_id).filter_by(accept_id=aid).scalar()), **self.get_params())
...
@permit.route('/admin/customer/acceptlog/export', u'用户受理日志导出', MenuUser, order=3.0001)
