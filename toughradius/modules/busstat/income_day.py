...
@permit.suproute('/admin/stat/income/day', u'营业日统计', MenuStat, order=1.00001, is_menu=True)
...
class IncomeDayStatHandler(BaseHandler):
    ...
    def post(self):
        ...
        _query = self.db.query(models.TrCustomer.node_id, models.TrNode.node_name, models.TrCustomerOrder.product_id, models.TrProduct.product_name, models.TrAcceptLog.operator_name, func.sum(models.TrCustomerOrder.actual_fee).label('actual_fee'), func.count(models.TrCustomerOrder.order_id).label('dev_count'), models.TrCustomerOrder.stat_day).filter(models.TrCustomerOrder.product_id == models.TrProduct.id, models.TrCustomerOrder.customer_id == models.TrCustomer.customer_id, models.TrNode.id == models.TrCustomer.node_id, models.TrAcceptLog.id == models.TrCustomerOrder.accept_id)
        if node_id:
        ...
        _query = _query.group_by('stat_day', 'node_id', 'node_name', 'product_id', 'product_name', 'operator_name')
        ...
        if self.request.path == '/admin/stat/income/day':
            return self.render('income_day_stat.html', node_list=opr_nodes, fee_total=fee_total, dev_total=dev_total, products=opr_products, statitems=_query.limit(5000), **self.get_params())
        elif self.request.path == '/admin/stat/income/day/export':
            data = Dataset()
            data.append((u'日期', u'区域', u'资费', u'操作员', u'受理用户数', u'总金额'))
            _f2y = utils.fen2yuan
            for i in _query:
