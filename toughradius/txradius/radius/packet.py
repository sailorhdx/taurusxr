...
class Packet(dict):
    ...
    def __init__(self, code = 0, id = None, secret = six.b(''), authenticator = None, **attributes):
        ...
        self.secret = secret
        if authenticator is not None and not isinstance(authenticator, six.binary_type):
    ...
    def CreateReply(self, **attributes):
        ...
        to the new instance.
        """
        return Packet(id=self.id, secret=self.secret, authenticator=self.authenticator, dict=self.dict, **attributes)
    ...
    def VerifyReply(self, reply, rawreply = None):
        ...
        hash = md5_constructor(rawreply[0:4] + self.authenticator + rawreply[20:] + self.secret).digest()
        ...
        if len(data) < 6:
            ...
            while len(data) > 2:
                ...
                if len(data) < length or length < 3:
                    raise PacketError('SubAttribute has invalid length: {0}, actual={1}'.format((vendor, type_, length), len(data)))
...
class AuthPacket(Packet):
    ...
    def CreateReply(self, **attributes):
        ...
        to the new instance.
        """
        return AuthPacket(AccessAccept, self.id, self.secret, self.authenticator, dict=self.dict, **attributes)
...
class AcctPacket(Packet):
    ...
    def RequestPacket(self):
        ...
        if self.id is None:
            self.id = self.CreateID()
        header = struct.pack('!BBH', self.code, self.id, 20 + len(attr))
        self.authenticator = md5_constructor(header[0:4] + 16 * six.b('\x00') + attr + self.secret).digest()
...
class CoAPacket(Packet):
    ...
    def CreateReply(self, **attributes):
        ...
        to the new instance.
        """
        return CoAPacket(CoAACK, self.id, self.secret, self.authenticator, dict=self.dict, **attributes)
